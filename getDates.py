...
class Course:
    def __init__(self, name, section, instructor, start_date, end_date, start_time, end_time, location,
                 meeting_patterns, days):
        self.name = name
...
def main(filename):
    csv_titles = ["Subject", "Description", "Start Time", "End Time", "Location", "Start Date", "End Date",
                  "All Day Event", "Private"]  # csv headers
    ...
    for row in class_row:
        ...
        classes.append(
            Course(name, section, instructor, start_date, end_date, start_time, end_time, location, meeting_patterns,
                   days))  # create a class object with the attributes read from the workday file

    rows_to_write = []
    for c in classes:
        start_date = datetime.date(int(c.start_date.split("-")[0]), int(c.start_date.split("-")[1]),
                                   int(c.start_date.split("-")[2]))  # gets the start day of each course
        date_iter = start_date  # creates a variable to iterate through dates
        end_date = datetime.date(int(c.end_date.split("-")[0]), int(c.end_date.split("-")[1]),
                                 int(c.end_date.split("-")[2]))  # gets the end day of each course
        delta = datetime.timedelta(days=1)  # increments the date by one day
        while date_iter <= end_date:
            if (getDayFromDate(
                    date_iter) in c.days):  # if the day of the week is in the days of the course ex. M,T,W,R,F
                row = f'{c},{c.section},{c.start_time},{c.end_time},{c.location}'  # get the row to write to the csv
                print(row)
                rows_to_write.append(
                    [c.name, c.section, c.start_time, c.end_time, c.location, date_iter.strftime("%m/%d/20%y"),
                     date_iter.strftime("%m/%d/20%y"), "False", "False"])  # add the row to the list of rows to write
            date_iter += delta

    # write the rows to the csv

    with open(os.path.join(os.path.join(os.getcwd(), 'downloads'), 'classes.csv'), 'w+', newline='') as csvfile:
